__Malaria Incidence Analysis, Nigeria__

__What this does:__

Analyses malaria data in Nigeria from the WHO.

__Findings__

Linear model predicts 235.29 cases per 1000 by 2030.

__Methods Used__

_Data Cleaning_

_Statistics_: Linear Regression

_Visualization_: Matplotlib

__Limitation__

Linear regression ignores factors like the seasons and malaria interventions that affect malaria incidences.

Uncertainty increases with predictions.

While there is a fluctuatiion in real malaria trends, linear regression assumes a constant rate of change.

__Files__

Nigeria_malaria_incidence.ipynb - Jupyter notebook

incidence-of-malaria.csv - data from WHO

__Source__

WHO Global Health Observatory through the World Bank
